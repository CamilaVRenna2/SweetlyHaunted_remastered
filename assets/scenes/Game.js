export default class Game extends Phaser.Scene {
    ...
    init(data) {
      ...
      create() {
        if (this.level>=1){const map = this.make.tilemap({ key: "map1" });}
        else if (this.level>=2) {const map = this.make.tilemap({ key: "map2" });}
        else if (this.level>=3) {const map = this.make.tilemap({ key: "map3" });}
